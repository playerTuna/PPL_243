...
class Emitter:
    ...
    def get_full_type(self, in_type) -> str:
        ...
        if type_in is IntType:
            ...
            return self.get_jvm_type(in_type)
    ...
    def emit_ificmple(self, label: int, frame) -> str:
        """
        Emit IFICMPLE instruction.
    ...
    def emit_return(self, in_, frame) -> str:
        ...
        if type(in_) is IntType or type(in_) is BoolType:
            ...
            type(in_) is ArrayType
            or type(in_) is ClassType
            or type(in_) is StringType
        ):
            frame.pop()
            return self.jvm.emitARETURN()

    def emit_new_array(self, typ: str, frame: Frame) -> str:
        """
        ...
        if typ in ["int", "float", "boolean"]:
            return self.jvm.emitNEWARRAY(typ)
        else:
            return self.jvm.emitANEWARRAY(typ)
        
    def emit_multianewarray(self, typ: str, dimensions: int, frame: Frame) -> str:
        """
        Emit MULTIANEWARRAY instruction.

        Args:
            typ: Array type string
            dimensions: Number of dimensions for the array
            frame: Frame object for stack management
    ...
    def emit_push_null(self, frame: Frame) -> str:
        """
        Emit instruction to push null onto the operand stack.
